...
def isIrreducible(polynomial, p):
    ...
    def isUnit(p): return p.degree() == 0
...
def generateIrreduciblePolynomial(modulus, degree):
    ...
    while True:
        coefficients = [Zp(random.randint(0, modulus-1))
...
def FiniteField(p, m, polynomialModulus=None):
    ...
    class Fq(FieldElement):
        ...
        def __eq__(self, other): return isinstance(
            other, Fq) and self.poly == other.poly
        ...
        def inverse(self):
            ...
            x, y, d = extendedEuclideanAlgorithm(
                self.poly, self.idealGenerator)
            if d.degree() != 0:
                raise Exception(
