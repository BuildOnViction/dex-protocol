...
func main() {
	...
	genesisConfig := initGenesis(stack, genesisPath)
	...
	var currentBlockNumber = uint64(chain.CurrentBlock().Number().Int64())
	...
	input, _ := parsed.Pack("balanceOf", userAddress)
	...
	// 			fmt.Printf("Key: %s, block number: %d, iterator: %d\n", key.Hex(), i, itIndex)
	// 			printJSON(account)
	// 		}
